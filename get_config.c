...
struct prog_config get_program_config() 
{
	...
	if (config_hold.support_userpasswd)
	{
		// get the credentials
		if (!config_lookup_string(cf, "mqtt_username", &config_hold.mqtt_username))
		{
			printf("could not find the mqtt username\n");
		}
		
		if (!config_lookup_string(cf, "mqtt_password", &config_hold.mqtt_password))
		{
			printf("could not find the mqtt password\n");
		}
	}
